...
| dart_symbol_map_path     | Absolute or relative path to a Dart obfuscation map file to upload alongside relevant Flutter debug files. The map file is generated by using `--obfuscate` and `--extra-gen-snapshot-options=--save-obfuscation-map=<path>` during Flutter build. | null (string)                                                 | no       | SENTRY_DART_SYMBOL_MAP_PATH      |
